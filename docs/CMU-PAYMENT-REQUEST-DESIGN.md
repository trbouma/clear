# CMU Payment Request Design

Status: Proposed for the first release

## Summary

Clear should implement a profile of Cashu NUT-18 Payment Requests for Mint
Notes denominated in a specific `cmu-<keyset-id>`. It should not invent a new
wire format. A compatible request retains the standard `creqA` prefix,
CBOR serialization, Base64URL encoding, payment payload, and transport model.
...
`sm` is omitted in the first release. A Clear CMU payment is a direct transfer
...
Before accepting the payment, the receiver must:

1. decode the request or payload using bounded input sizes;
2. match the payment `id` to an outstanding request when `i` was supplied;
3. require the payment `unit` to equal the request `u` exactly;
...
also establish that the proofs have not already been spent. A payment shown as
received but not yet refreshed should remain pending rather than final.
...
- A payment request is not proof of payment, an invoice settlement receipt, or
...
- accept and refresh a valid payment from the requested CMU;
