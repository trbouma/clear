...
Clear should host multiple independent currencies without becoming the
custodian of treasurer private keys. Each currency has its own durable identity,
operational keysets, treasury policy, ledger, and authorized treasurer public
keys.
...
single-use authorization events for actions such as issuance and retirement.
...
individual issuance and retirement authorizations within their scopes, limits,
and approval threshold. They cannot appoint themselves, alter the threshold,
replace the mint identity, or redefine the currency.
...
Mint Operator installs policy and restarts Clear
  ...
  v
Treasurers authorize issuance and retirement
```
...
| Treasurer | Treasurer wallet or hardware signer | Individual issuance and retirement authorizations |
...
The currency root defines the durable currency identity and authorizes treasury
policy and operational keysets. It should normally be offline or
hardware-backed.

The protocol unit should be bound to the currency root fingerprint:

```text
pts.<currency-root-fingerprint>
```

This lets a currency rotate operational keysets without changing its durable
identity. It supersedes the initial prototype model that bound the unit directly
to one operational keyset.
...
Treasurers authorize administrative actions, but their keys do not sign Cashu
proofs and cannot independently create cryptographically valid ecash.

### Operational mint signer

Operational denomination keys sign blinded Cashu outputs and validate proofs.
They should be replaceable keyset epochs authorized by the currency root.
...
The friendly name can change without changing currency identity. The root
fingerprint, protocol unit, mint URL, and keyset identifiers remain the
...
3. Persist the complete signed event, policy version, and event ID in the
   currency ledger.
4. Bind the initialized ledger to the currency root and protocol unit.
...
- a **service record** containing the public mint identity, endpoints, relay
  hints, protocol unit, and active Cashu keysets.
...
```json
{
  ...
  "action": "issue",
  "amount": 500,
  "unit": "pts.<currency-root-fingerprint>",
...
currency profiles, creates blinded outputs, signs authorizations, tracks
pending threshold approvals, verifies mint responses, and constructs returned
Cashu tokens locally. It must never import the mint database, operational
signing keys, or an internal authorization bypass.

For a one-of-one policy, the initial experience should be:

```text
clear-treasury issue \
  --currency pts.<currency-root-fingerprint> \
  --amount 500 \
...
secrets. The resulting token belongs to exactly one protocol unit and must not
be combined with another currency.

Threshold workflows separate request creation, approval, and claiming:

```text
clear-treasury issue-request --currency <unit> --amount 500
...
Retirement must bind the approval to a commitment over the exact proof set,
not merely an amount. Clear should calculate a canonical commitment from the
currency ID, keyset IDs, proof `Y` values, amounts, and intended reason.
...
The current Clear prototype has one keyset-bound unit, one SQLite ledger, and a
global operator token. Migration should occur before any meaningful proofs are
issued.
...
### Phase 2: Durable currency root

- Introduce an offline currency root identity.
- Bind the protocol unit to the currency root rather than an operational
  keyset.
